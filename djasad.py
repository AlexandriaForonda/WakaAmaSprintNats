...
directory_paths_2018 = [
    r'C:\Users\lisia\Downloads\WakaNats2018'
]

directory_paths_2017 = [
    r'C:\Users\lisia\Downloads\WakaNats2017'
]

# Create a dictionary to store total scores for each name
total_scores_by_name = {}

# Process directory paths for 2018
for directory_path in directory_paths_2018:
    for root, dirs, files in os.walk(directory_path):
        for file in files:
            if os.path.isfile(os.path.join(root, file)) and os.path.splitext(file)[1].endswith('.csv') and 'Final' in file:
                print("Processing 2018 file:", os.path.join(root, file))  # Print the file being processed
...
# Process directory paths for 2017
for directory_path in directory_paths_2017:
    for root, dirs, files in os.walk(directory_path):
        for file in files:
            if os.path.isfile(os.path.join(root, file)) and os.path.splitext(file)[1].endswith('.csv') and 'Final' in file:
                print("Processing 2017 file:", os.path.join(root, file))  # Print the file being processed
